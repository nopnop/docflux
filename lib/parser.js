...
function parser(raw, firstLine) {
  ...
  .filter(function(tag) { return tag.tag == 'param' })
  .map(function(tag) { return tag.token  })
  .join(', ')


  if(!/[\.#]/.test(result.token)) {
    var memberOf = _.findWhere(result.tags, {'tag':'memberOf'});
    if(memberOf) {
  ...
  result.flags   = _.uniq(_.pluck(result.tags,'token'))
